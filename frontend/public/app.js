...
function syncSidebar() {
  ...
  sb.querySelector("#sb-map")?.classList.toggle("on", !state.openState && !pol && !dw && !prj);
  ...
  sb.querySelectorAll("[data-sb-state]").forEach((b) => {
    const isOpen = !pol && !dw && !prj && state.openState === b.dataset.sbState;
...
document.getElementById("sidebar")?.addEventListener("click", (ev) => {
  ...
  if (ev.target.closest("#sb-brand") || ev.target.closest("#sb-map")) {
    closePoliticians({ silent: true }); closeNewsPage({ silent: true }); closeDewanPage({ silent: true }); closeProjectionPage({ silent: true }); hideInfo(); backToControls(); syncSidebar(); return;
  }
