...
const WORKER_ORIGIN = "https://politikku.ilhamkassim2003.workers.dev";
...
test("worker: GET /api/live/prn16-johor falls back to asset with suffix match", async () => {
  const req = new Request(`${WORKER_ORIGIN}/api/live/prn16-johor`);
  const resp = await worker.fetch(req, makeEnv(new MemoryKV()));
  assert.equal(resp.status, 200);
  const data = await resp.json();
  assert.equal(data.phase, "final");
...
test("worker: GET /api/live/<unknown> returns campaign default", async () => {
  const req = new Request(`${WORKER_ORIGIN}/api/live/prn17-melaka`);
  const resp = await worker.fetch(req, makeEnv(new MemoryKV()));
  assert.equal(resp.status, 200);
  const data = await resp.json();
...
test("worker: PUT /api/live/:electionId rejects mismatched election payload", async () => {
  const kv = new MemoryKV();
  const env = makeEnv(kv, "secret-key");
  const req = new Request(`${WORKER_ORIGIN}/api/live/prn17-sarawak`, {
    method: "PUT",
    headers: {
...
test("worker: PUT /api/live/:electionId updates KV and serves on subsequent GET", async () => {
  ...
  const putReq = new Request(`${WORKER_ORIGIN}/api/live/prn17-melaka`, {
    method: "PUT",
    headers: {
      "Authorization": "Bearer secret-key",
      "Content-Type": "application/json",
  ...
  const getReq = new Request(`${WORKER_ORIGIN}/api/live/prn17-melaka`);
...
test("worker: legacy PUT /api/live/johor accepts prn16-johor payload and syncs aliases", async () => {
  ...
  const putReq = new Request(`${WORKER_ORIGIN}/api/live/johor`, {
    method: "PUT",
    headers: {
  ...
  const getReq1 = new Request(`${WORKER_ORIGIN}/api/live/johor`);
  ...
  const getReq2 = new Request(`${WORKER_ORIGIN}/api/live/prn16-johor`);
