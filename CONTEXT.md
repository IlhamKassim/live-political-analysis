...
**MP Profile**:
The sitting Member for a Seat and their record this term — identity, Coalition, GE15 result, contact details, Divisions voted in, Bills sponsored (#78). Built in `lpa.mp_profile`/`lpa.config.load_mp_profiles` from `data/mp_profiles.json`; see ADR 0009 for what each official source does and does not publish. A field with no value must say why it has none: the profile carries an `unverified` block naming every unset field and the reason, and the loader rejects a profile that leaves one unexplained. The Member is identified by `seat_code` alone — name, state and demographics for the Seat live on its Baseline. The same discipline applies to a Seat with no profile at all: `data/mp_profiles.json` accounts for all 222, and a Seat the official sources cannot support one for is in its `_skipped` block with the reason rather than silently absent (#105).
...
**Bill**:
A piece of legislation before the Dewan Rakyat, tracked on PolitikKu's homepage bill tracker (#80, scaled to the register's full default view by #106): title, year, stage, the date of that stage, a plain-language summary, and a Division result where one exists. Built in `lpa.bill_tracker`/`lpa.config.load_bills` from `data/bills.json`; see ADR 0010 for how it is sourced. `stage` is Parliament's own status label (*Lulus*, *Dirujuk ke JKPK*, …), not a translated or invented category; `summary` is a verbatim excerpt of the Bill's own "HURAIAN" explanatory statement, not a paraphrase this pipeline wrote. Identified by `code` alone (Parliament's own D.R. reference), the same discipline as `SeatCall`/`SeatMatch`/`MPProfile`. A Bill's Division result is absent for most Bills — see Division — and that absence carries a reason the same way `MPProfile.unverified` does.
...
**As of ADR 0017, the site root is an orientation page again, and it carries the Postcode → Seat Index lookup.** ADR 0014 had made the root the `/app/` SPA, which left Audience with no dedicated served surface at all — the old homepage/landing, postcode lookup and all, had been Audience's on-ramp. ADR 0017 puts a general-audience landing page back at `/` (the SPA moves to `/app/`), and the lookup is its primary action, so a #40 Telegram Seat Call card click — Audience's actual discovery channel — now lands on a page that opens with "find your Seat" rather than on Engaged-Reader content.

**That is a restored surface, not a closed gap.** The landing page is built for a general, undifferentiated first-time visitor and assumes nothing about how anyone arrived; it is not built *for* Audience specifically. Audience's remaining work — #22 (site-literacy) and #23 (shareable cards) — is still unscheduled. Do not read the presence of a postcode field on the root as the Audience question being answered.
_Avoid_: Users, readers (both too generic — use Audience when the younger, secondhand-discovery reader is specifically meant)

**Engaged Reader**:
A reader who already follows Malaysian politics and comes to this site by deliberate navigation, not secondhand discovery — the counterpart Audience is explicitly defined against. Served by the `/projection/` detail page (the full seat ledger, sensitivity table, Majority-margin trend, per-state rollup, methodology, and the dated-permalink citation archive — see ADR 0011) **and by the `/app/` map SPA** — which was the site root under ADR 0014 and moved to its own `/app/` path under ADR 0017. An Engaged Reader reaches it in one click from the landing page at `/`, or directly at `/app/`; a `/` URL carrying a deep-link fragment forwards there immediately. ADR 0017 briefly skipped the landing page for returning visitors and no longer does, so `politikku.my` itself is one extra click away from the map — named here because it is a real cost to this reader specifically.
